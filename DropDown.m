...
#define kDefaultDropDownViewColor [UIColor colorWithWhite:0.4 alpha:1.0]
#define kDefaultDropDownHeight 80.0f
#define kDefaultFontSize 13.0f
#define kDefaultDropDownTableViewCellHeight 40.0f
#define kDefaultGapBetweenDropDownAndViewForDropDown 0.5f
...
@interface DropDown ()<UITableViewDataSource,UITableViewDelegate,UIGestureRecognizerDelegate>
{
...
-(void)showDropDownForView:(UIView *)viewForDropDown insideTheView:(UIView *)superView animation:(BOOL)animate
{
    ...
    if (!self.dropDownHeight || self.dropDownHeight == 0.0f)
    {
         ...
         */
        self.dropDownHeight = kDefaultDropDownHeight;
    }
    ...
    {
        self.backgroundColor = kDefaultDropDownViewColor;
    }
    ...
    else
    {
        ...
        [appObject.window.rootViewController.view addSubview:viewBackground];
    }
    
    CGPoint point = [superView convertPoint:viewForDropDown.frame.origin toView:viewBackground];
    if (self.dropDownPosition == kDropDownPositionBottom)
    {
    ...
    else if (self.dropDownPosition == kDropDownPositionTop)
    {
    ...
    else if (self.dropDownPosition == kDropDownPositionLeft)
    {
    ...
    else if (self.dropDownPosition == kDropDownPositionRight)
    {
...
-(void)creatShape
{
    ...
    if (_needToShowArrow)
    {
         ...
         */
        CGRect frameTable;
        if (self.dropDownPosition == kDropDownPositionLeft)
        {
            frameTable = CGRectMake(0, 0, self.dropDownWidth - arrowWidth, self.dropDownHeight);
        }
        else if (self.dropDownPosition == kDropDownPositionBottom)
        {
    ...
    else
    {
        ...
        if (self.dropDownPosition == kDropDownPositionLeft)
        {
            [self setFrame:CGRectMake(self.frame.origin.x - kDefaultGapBetweenDropDownAndViewForDropDown, self.frame.origin.y, self.frame.size.width, self.frame.size.height)];
        }
        else if (self.dropDownPosition == kDropDownPositionBottom)
        {
            
            
            if (_WAFIERSPECIALCASEBottomCornerRadius)
            {            
                UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height) byRoundingCorners:UIRectCornerBottomLeft|UIRectCornerBottomRight cornerRadii:CGSizeMake(_WAFIERSPECIALCASEBottomCornerRadius, _WAFIERSPECIALCASEBottomCornerRadius)];
                shapeLayer = [CAShapeLayer layer];
                ...
                self.backgroundColor = [UIColor clearColor];
                [self.layer addSublayer:shapeLayer];
                self.tableViewDropDownList.frame = CGRectMake(self.tableViewDropDownList.frame.origin.x, self.tableViewDropDownList.frame.origin.y, self.tableViewDropDownList.frame.size.width, self.tableViewDropDownList.frame.size.height - _WAFIERSPECIALCASEBottomCornerRadius);
            }
            else
            {
                [self setFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + kDefaultGapBetweenDropDownAndViewForDropDown, self.frame.size.width, self.frame.size.height)];
            }
        }
        else if (self.dropDownPosition == kDropDownPositionTop)
        {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        return kDefaultDropDownTableViewCellHeight;
    }
}
